...
function AccountStatus(props) {
  ...
  React.useEffect(() => {
    ...
    props.AccountStatus({token: props.auth.token, t: props.t})
    .then(res=>{
        if(res.detail){
    ...
    return (
        <Box className={classes.rootStyle}>
            <Container className={classes.mainContainerStyle}>
                ...
                <Typography className={classes.messageStyle} variant="h3">{message}</Typography>
                <Typography className={classes.messageStyle} variant="h6">{t('accountStatus.contactAdmin')}</Typography>
                <CustomButton
