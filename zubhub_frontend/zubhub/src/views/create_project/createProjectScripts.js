...
export const handleDescFieldChange = (value, props, handleSetState) => {
  if(value && value !== '<p><br></p>'){ // second conditional is a guide for when quill is cleared.
...
export const buildPublishTypes = ({projects, values, setFieldValue}) => {
  ...
  if(zubhub?.site_mode === site_mode.PRIVATE){
    publish_types = {publish_types:[
                      {value: publish_type["Authenticated Creators"], name: "Authenticated Creators"},
...
export const handleAddPublishVisibleTo = (e, props, publish_visible_to_el) => {
  ...
  const exists =
    publish.visible_to.filter(username => username === value[0]).length > 0 ? true : false;
...
export const suggestPublishVisibleTo = (value, props, handleSetState, _) => {
  ...
  const res = props.suggestCreators({
        page: null, query_string: value, t: props.t 
        }).then(res=>{
...
export const validationSchema = Yup.object().shape({
  ...
  publish: Yup.mixed().test('visible_to_required','visible_to_required', publish => {
      if(publish.type === publish_type["Preview"] && 
